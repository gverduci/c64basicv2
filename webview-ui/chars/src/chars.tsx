...
const handlerMessage = (
  event: IEvent,
  command: string,
  callback: (data: ICharObject[] | undefined) => void,
) => {
...
function Chars() {
  ...
  useEffect(() => {
    subscribeToMessage("getCharsResponse", (data: ICharObject[] | undefined) =>
      setChars(data),
    );
    vscode.postMessage({
      command: "getChars",
              ...
              <VSCodeDataGridRow>
                <VSCodeDataGridCell
                  ...
                  style={{
                    paddingLeft: 0,
                    cursor: "pointer",
                    font: 'normal 13px "PetMe64"',
                  }}
                  onClick={() => handleCharacterClick(char.item.symbolic)}
                >
                  {char.item.unicode}
                </VSCodeDataGridCell>
                <VSCodeDataGridCell
                  ...
                  onClick={() => handleCharacterClick(`chr$(${char.item.id})`)}
                >
                  {char.item.id}
                </VSCodeDataGridCell>
                <VSCodeDataGridCell
                ...
                >
                  <div style={{ display: "grid" }}>
                    <div> {label}</div>
                    <div style={{ fontSize: "x-small", whiteSpace: "normal" }}>
