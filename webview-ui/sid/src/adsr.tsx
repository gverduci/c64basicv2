...
function ADSR() {
  ...
  const [release, setRelease] = useState<number>(1.5);
    ...
    const drawADSR = (
      ...
      if (canvasContext) {
        ...
        ctx.quadraticCurveTo(
          sustainx + (releasex - sustainx) / 12,
          bottomy + (bottomy - sustainy) / 12,
          releasex,
          bottomy,
        );
        ctx.stroke();
        ...
        ctx.strokeText(
          "A",
          attackp - (attackp - basex) / 2 - headlen,
          bottomy + 2 * headlen,
        );
        ...
        ctx.strokeText(
          "D",
          decayp - (decayp - attackp) / 2 - headlen,
          bottomy + 2 * headlen,
        );
        ...
        ctx.strokeText(
          "S",
          sustainx - (sustainx - decayp) / 2 - headlen,
          bottomy + 2 * headlen,
        );
        ...
        ctx.strokeText(
          "R",
          releasex - (releasex - sustainx) / 2 - headlen,
          bottomy + 2 * headlen,
        );
        ...
        ctx.strokeText(
          "Gate",
          sustainx - (sustainx - basex) / 2 - headlen,
          bottomy + bottomAreaGate + 1 * headlen,
        );
        ...
        if (sustainy !== peaky) {
          ctx.strokeText(
            "Peak",
            startx + headlen,
            peaky - headlen / 2 + headlen,
          );
        }
        ctx.translate(
          startx - 2 * headlen,
          (bottomy - peaky) / 2 + headlen - 35,
        );
        ...
        if (sustainy !== bottomy) {
          ctx.strokeText(
            "Sl",
            startx + headlen,
            sustainy - headlen / 2 + headlen,
          );
        }
        ctx.stroke();
      }
    };
    if (canvasContext) drawADSR(attack, decay, sustainLevel, release, sustain);
  }, [
    attack,
    decay,
    sustainLevel,
  ...
  const attackValues = [
    ...
    { value: "7", dataValue: 0.08, label: "7 (0111) - A: 80ms" },
    { value: "8", dataValue: 0.1, label: "8 (1000) - A: 100ms" },
    { value: "9", dataValue: 0.25, label: "9 (1001) - A: 250ms" },
    { value: "10", dataValue: 0.5, label: "10 (1010) - A: 0.5s" },
    { value: "11", dataValue: 0.8, label: "11 (1011) - A: 0.8s" },
    { value: "12", dataValue: 1.0, label: "12 (1100) - A: 1s" },
    { value: "13", dataValue: 3.0, label: "13 (1101) - A: 3s" },
    { value: "14", dataValue: 5.0, label: "14 (1110) - A: 5s" },
    { value: "15", dataValue: 8.0, label: "15 (1111) - A: 8s" },
  ];
  const handleAttackChange = (evt: Event | React.FormEvent<HTMLElement>) => {
    const element = evt.target as HTMLInputElement;
    setAttackSelected(element.value);
    setAttack(
      attackValues.find((r) => r.value === element.value)?.dataValue || 0,
    );
  };
  ...
  const decayValues = [
    ...
    { value: "7", dataValue: 0.24, label: "7 (0111) - D: 240ms" },
    { value: "8", dataValue: 0.3, label: "8 (1000) - D: 300ms" },
    { value: "9", dataValue: 0.75, label: "9 (1001) - D: 750ms" },
    { value: "10", dataValue: 1.5, label: "10 (1010) - D: 1.5s" },
    { value: "11", dataValue: 2.4, label: "11 (1011) - D: 2.4s" },
    { value: "12", dataValue: 3.0, label: "12 (1100) - D: 3.0s" },
    { value: "13", dataValue: 9.0, label: "13 (1101) - D: 9.0s" },
    { value: "14", dataValue: 15.0, label: "14 (1110) - D: 15.0s" },
    { value: "15", dataValue: 24.0, label: "15 (1111) - D: 24.0s" },
  ];
  const handleDecayChange = (evt: Event | React.FormEvent<HTMLElement>) => {
    const element = evt.target as HTMLInputElement;
    setDecaySelected(element.value);
    setDecay(
      decayValues.find((r) => r.value === element.value)?.dataValue || 0,
    );
  };
  ...
  const handleSustainChange = (evt: Event | React.FormEvent<HTMLElement>) => {
    ...
    setSustainLevel(
      sustainLevelValues.find((r) => r.value === element.value)?.dataValue || 0,
    );
  };
  ...
  const releaseValues = [
    ...
    { value: "7", dataValue: 0.24, label: "7 (0111) - R: 240ms" },
    { value: "8", dataValue: 0.3, label: "8 (1000) - R: 300ms" },
    { value: "9", dataValue: 0.75, label: "9 (1001) - R: 750ms" },
    { value: "10", dataValue: 1.5, label: "10 (1010) - R: 1.5s" },
    { value: "11", dataValue: 2.4, label: "11 (1011) - R: 2.4s" },
    { value: "12", dataValue: 3.0, label: "12 (1100) - R: 3.0s" },
    { value: "13", dataValue: 9.0, label: "13 (1101) - R: 9.0s" },
    { value: "14", dataValue: 15.0, label: "14 (1110) - R: 15.0s" },
    { value: "15", dataValue: 24.0, label: "15 (1111) - R: 24.0s" },
  ];
  ...
  const handleReleaseChange = (evt: Event | React.FormEvent<HTMLElement>) => {
    ...
    setRelease(
      releaseValues.find((r) => r.value === element.value)?.dataValue || 0,
    );
  };
  const handleADClick = () => {
      ...
      <div style={{ display: "grid", gridTemplateColumns: "auto auto auto" }}>
        <div style={{ display: "grid" }}>
          ...
          <VSCodeDropdown
            ...
            value={attackSelected}
            style={{ height: "28px", width: "150px" }}
          >
            {attackValues.map((r) => (
              <VSCodeOption key={`d${r.value}`} value={`${r.value}`}>
                {r.label}
              </VSCodeOption>
            ))}
          </VSCodeDropdown>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "auto auto auto" }}>
        <div style={{ display: "grid" }}>
          ...
          <VSCodeDropdown
            ...
            value={sustainLevelSelected}
            style={{ height: "28px", width: "150px" }}
          >
            {sustainLevelValues.map((r) => (
              <VSCodeOption key={`s${r.value}`} value={`${r.value}`}>
                {r.label}
              </VSCodeOption>
            ))}
          </VSCodeDropdown>
        ...
        <div style={{ marginLeft: "16px", display: "grid" }}>
          ...
          >
            {releaseValues.map((r) => (
              <VSCodeOption key={`r${r.value}`} value={`${r.value}`}>
                {r.label}
              </VSCodeOption>
            ))}
          </VSCodeDropdown>
